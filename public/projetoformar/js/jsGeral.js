...
function redirecionarTela(tela){
	if(tela == 1){
		...
		$(window).attr('location','meusDadosFornecedor.html');
	}else if (tela == 6){
		$(window).attr('location','meusDadosFornecedor.html');
	}else if (tela == 7){
		$(window).attr('location','meusDadosFornecedor.html');
...
function obterDescricaoPerfil(id){
		...
		function() {
			...
			if(navigation != null){
			    ...
			    if(s.indexOf(',,,,') == -1){
				    ...
				    $('#perfilSelecionado').empty();
				    $('#perfilSelecionado').append('<font color="yellow">' + obterDescricaoPerfil(perfil_) + '</font>');			    	
			    }
			    $('#emailUsuario').empty();
			    $('#lbUsuario').empty();
...
function cutStringMobile(str,tipo){
